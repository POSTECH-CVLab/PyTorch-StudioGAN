...
class Generator(nn.Module):
    ...
    def __init__(self, z_dim, shared_dim, g_conv_dim, g_spectral_norm, attention, at_after_th_gen_block, leaky_relu,
        ...
        if g_spectral_norm:
            self.linear0 = snlinear(in_features=self.chunk_size, out_features=g_conv_dim*16*4*4)
        else:
            self.linear0 = linear(in_features=self.chunk_size, out_features=g_conv_dim*16*4*4)
