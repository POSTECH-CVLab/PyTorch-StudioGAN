...
class Generator(nn.Module):
    ...
    def __init__(self, z_dim, shared_dim, g_conv_dim, g_spectral_norm, attention, at_after_th_gen_block, leaky_relu,
        ...
        if g_spectral_norm:
            self.linear0 = snlinear(in_features=self.z_dim, out_features=self.in_dims[0]*3*3)
        else:
            self.linear0 = linear(in_features=self.z_dim, out_features=self.in_dims[0]*3*3)
