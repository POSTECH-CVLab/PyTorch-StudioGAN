""" This code is broght with slight modification from https://github.com/NVlabs/stylegan2-ada-pytorch
...
from util.ops import fma


#########################
import utils.style_misc as style_misc
...
def modulated_conv2d(x, weight, styles, noise=None, up=1, down=1, padding=0, resample_filter=None, demodulate=True, flip_weight=True, fused_modconv=True):
    ...
    out_channels, in_channels, kh, kw = weight.shape  # kernel height, kernel width
    style_misc.assert_shape(weight, [out_channels, in_channels, kh, kw])
    style_misc.assert_shape(x, [batch_size, in_channels, None, None])
    style_misc.assert_shape(styles, [batch_size, in_channels])
    ...
    with style_misc.suppress_tracer_warnings():  # this value will be treated as a constant
...
class FullyConnectedLayer(nn.Module):
    def __init__(self, in_features, out_features, bias=True, activation="linear", lr_multiplier=1, bias_init=0):
        ...
        self.bias = torch.n.Parameter(torch.full(
    ...
    def forward(self, x):
        ...
        if self.activation == "linear" and b is not None:
            x = torch.addmm(b.unsuqeeze(0), x, w.t())  # x = bias + x * w
        else:
            x = x.matmul(w.t())
...
class MappingNetwork(nn.Module):
    def __init__(self, z_dim, c_dim, w_dim, num_ws, num_layers=8, embed_features=None, layer_features=None, activation='lrelu', lr_multiplier=0.01, w_avg_beta=0.995):
        ...
        if embed_features is None:  # Label embedding dimensionality, None = same as w_dim
            embed_features = w_dim
        # Conditioning label (C) dimensionality, 0 = no label.
        if c_dim == 0:
            embed_features = 0
        if layer_features is None:  # Number of intermidiate features in the mapping layers, None = same as w_dim
            layer_features = w_dim
        # [input_dim, layer_features, layer_features, .., layer_features, w_dim]
    ...
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False):
        ...
        x = None
        if self.z_dim > 0:
            style_misc.assert_shape(z, [None, self.z_dim])
            x = normalize_2nd_moment(z.to(torch.float32))   # Normalize
        if self.c_dim > 0:
            style_misc.assert_shape(c, [None, self.c_dim])
...
class SynthesisLayer(nn.Module):
    ...
    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1):
        ...
        style_misc.assert_shape(x, [None, self.weight.shape[1],
    ...
    def __init__(self, in_channels, out_channels, w_dim, resolution, img_channels, is_last, architecture='skip', resample_filter=[1, 3, 3, 1], conv_clamp=None, use_fp16=False, fp16_channels_last=False, **layer_kwargs):
        ...
        if in_channels != 0 and architecture == 'resnet':
            self.skip = Conv2dLayer(in_channels, out_channels, kernel_size=1, bias=False,
                                    up=2, resample_filter=resample_filter, channels_last=self.channels_last)

    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, **layer_kwargs):
        style_misc.assert_shape(
        ...
        if self.in_channels == 0:
            ...
            style_misc.assert_shape(x, [None, self.in_channels,
        ...
        # ToRGB.
        if img is not None:
            style_misc.assert_shape(img, [None, self.img_channels,
                                          self.resolution // 2, self.resolution // 2])
...
class SynthesisNetwork(nn.Module):
    ...
    def forward(self, ws, **block_kwargs):
        ...
        with torch.autograd.profiler.record_function('split_ws'):
            style_misc.assert_shape(ws, [None, self.num_ws, self.w_dim])
...
class MinibatchStdLayer(torch.nn.Module):
    ...
    def forward(self, x):
        N, C, H, W = x.shape
        with style_misc.suppress_tracer_warnings():  # as_tensor results are registered as constants
...
class DiscriminatorEpilogue(torch.nn.Module):
    ...
    def forward(self, x, img, cmap, force_fp32=False):
        style_misc.assert_shape(
            x, [None, self.in_channels, self.resolution, self.resolution])  # [NCHW]
        _ = force_fp32  # unused
        dtype = torch.float32
        memory_format = torch.contiguous_format
        ...
        # Conditioning.
        if self.cmap_dim > 0:
            style_misc.assert_shape(cmap, [None, self.cmap_dim])
...
class Discriminator(nn.Module):
    def __init__(self, c_dim, img_resolution, img_channels, architecture='resnet', channel_base=32768, channel_max=512, num_fp16_res=0, conv_clamp=None, cmap_dim=None, block_kwargs={}, mapping_kwargs={}, epilogue_kwargs={}):
